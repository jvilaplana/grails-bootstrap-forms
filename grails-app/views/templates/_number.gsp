<div class="form-group form-group-${property} col-md-${width}${offset ? ' ' + offset : ''}${cssClass ? ' ' + cssClass : ''}${hidden == 'true' ? ' d-none' : ''}">
    ...
    <input type="number" step="${step}" ${min ? 'min="' + min + '"' : ''} max="${max ? max : ''}" name="${name}" value="${value}" ${required ? 'required="required"' : ''} class="form-control${height ? ' ' + height : ''}" id="${name}" placeholder="${message(code: domain + '.' + property + '.placeholder', default: message(code: domain + '.' + property + '.label'))}" tabindex="${tabindex}" />
    </g:applyCodec>
    <g:if test="${addon != null}">
